...
######################################################################################
def build_xml_tree__XGBClassifier(dt, node_id, node_pos, parent_depth, parent_elementTree):
	n_nodes = dt.tree_.node_count
	children_left = dt.tree_.children_left
	children_right = dt.tree_.children_right
	feature = dt.tree_.feature
...
def convert_bdt__XGBClassifier(sklearn_bdt_clf, input_var_list, tmva_outfile_xml):

    # classificator
	clf = sklearn_bdt_clf
	#clf = pickle.dumps(sklearn_bdt_clf)
	if clf.n_classes_ != 2: sys.exit("Error: Number of classes in sklearn classifier is not equal 2.")
	...
	for idx in range(NTrees):
		tree_weight = clf.evals_result()['validation_0']['logloss'][idx] #clf.estimator_weights_[itree]
        # <BinaryTree type="DecisionTree" boostWeight="9.2106320437773737e-01" itree="0">
		BinaryTree = ET.SubElement(Weights, "BinaryTree", type="DecisionTree", boostWeight=str(tree_weight), itree=str(idx))
...
def convert_bdt_sklearn_tmva(sklearn_bdt_clf, input_var_list, tmva_outfile_xml):
    ...
    # AdaBoost
    if 1>0 : #isinstance(sklearn_bdt_clf, XGBClassifier):
		print ("here XGB")
		convert_bdt__XGBClassifier(sklearn_bdt_clf, input_var_list, tmva_outfile_xml)
