...
def node_to_str_xgb(cls, tree, node_id, criterion, depth, kind, coef):
    if not isinstance(criterion, six.string_types):
        criterion = "impurity"
...
def xgbr_to_tmva(cls, evals_result, data, features, outfile_name, **kwargs): 
    ...
    <Option name="UseNvars" modified="No">{usenvars}</Option>
	...
	<Weights NTrees="{ntrees}" AnalysisType="1">
		""".format(**{"mva_name": mva_name,"ntrees":cls.n_estimators,"maxdepth":cls.max_depth,"usenvars":cls.n_estimators,"nvars": len(features),"varstring": varstring,"learnrate": cls.learning_rate})
	)
...
def gbr_to_tmva(cls, data, features, outfile_name, **kwargs):
    # if not isinstance(cls, GradientBoostingRegressor):
    #     raise ValueError("Can only export GradientBoostingRegressor")
    # if cls.loss != "huber":
    #     raise ValueError("TMVA assumes loss=huber")
